  ...
  return (
    <section ref={ref} className="relative overflow-hidden bg-[#050505] px-5 pb-[92px] pt-[96px] text-white sm:px-8 sm:pb-[108px] sm:pt-[112px] lg:px-12 lg:pb-[126px] lg:pt-[136px] xl:pb-[138px] xl:pt-[150px]">
      ...
      <div className="relative z-[2] mx-auto max-w-[1560px]">
        ...
        <div className="mt-[46px] lg:mt-[62px] lg:max-[1600px]:mt-[48px]">
          <div className="grid items-center gap-7 md:hidden">
            ...
            <motion.div
              className="mx-auto flex w-[220px] max-w-full items-center justify-center sm:w-[270px]"
          ...
          <div className="hidden md:block lg:hidden">
            <motion.div
              className="mx-auto mb-8 flex w-[240px] items-center justify-center"
              style={prefersReducedMotion ? undefined : { y: centerY, transformPerspective: 1100, transformStyle: 'preserve-3d' }}
              initial={prefersReducedMotion ? false : { opacity: 0, scale: 0.82, rotateY: -16 }}
              whileInView={prefersReducedMotion ? undefined : { opacity: 1, scale: 1, rotateY: 0 }}
              viewport={{ once: false, amount: 0.4 }}
            ...
            <div className="mx-auto grid max-w-[760px] grid-cols-2 gap-x-6">
          ...
          <div className="hidden items-center gap-8 lg:grid lg:grid-cols-[minmax(340px,1fr)_minmax(230px,360px)_minmax(340px,1fr)] xl:gap-12 xl:max-[1600px]:gap-8">
            ...
            <motion.div
              className="mx-auto flex w-[340px] max-w-full items-center justify-center lg:max-[1600px]:w-[300px] lg:max-[1400px]:w-[270px]"
              style={prefersReducedMotion ? undefined : { y: centerY, transformPerspective: 1100, transformStyle: 'preserve-3d' }}
              initial={prefersReducedMotion ? false : { opacity: 0, scale: 0.82, rotateY: -16 }}
              whileInView={prefersReducedMotion ? undefined : { opacity: 1, scale: 1, rotateY: 0 }}
              viewport={{ once: false, amount: 0.4 }}
...
function HomeWhyChooseItem({ item, iconFallback, index, align, compact = false }: { item: AboutUsItem; iconFallback: string; index: number; align: 'left' | 'right'; compact?: boolean }) {
  const prefersReducedMotion = useReducedMotion();
  ...
  return (
    <motion.article
      className={
        compact
          ? 'relative grid min-h-[74px] grid-cols-[52px_1fr] gap-2 py-[6px] after:absolute after:bottom-0 after:left-[68px] after:right-0 after:h-px after:bg-white/[0.09] first:before:absolute first:before:left-[68px] first:before:right-0 first:before:top-0 first:before:h-px first:before:bg-white/[0.09] sm:min-h-[80px] sm:grid-cols-[64px_1fr] sm:gap-3 sm:py-[7px] sm:after:left-[80px] sm:first:before:left-[80px]'
          : 'relative grid min-h-[78px] grid-cols-[78px_1fr] gap-4 py-[8px] after:absolute after:bottom-0 after:left-[94px] after:right-0 after:h-px after:bg-white/[0.09] first:before:absolute first:before:left-[94px] first:before:right-0 first:before:top-0 first:before:h-px first:before:bg-white/[0.09] sm:min-h-[86px] sm:grid-cols-[112px_1fr] sm:after:left-[128px] sm:first:before:left-[128px] lg:min-h-[92px] lg:py-[10px] lg:max-[1600px]:min-h-[76px] lg:max-[1600px]:grid-cols-[76px_1fr] lg:max-[1600px]:py-[7px] lg:max-[1600px]:after:left-[92px] lg:max-[1600px]:first:before:left-[92px]'
      }
    ...
    >
      <div className={`flex h-full items-center justify-center border-r border-white/[0.14] ${compact ? 'min-h-[56px] sm:min-h-[60px]' : 'min-h-[64px] lg:max-[1600px]:min-h-[56px]'}`}>
        <motion.div
        ...
        >
          <Image
            src={icon}
            alt={item.icon_alt ?? item.line_1 ?? ''}
            width={32}
            height={32}
            className={`object-contain brightness-0 invert ${compact ? 'h-[20px] w-[20px] sm:h-[22px] sm:w-[22px]' : 'h-[23px] w-[23px] sm:h-[26px] sm:w-[26px] lg:max-[1600px]:h-[22px] lg:max-[1600px]:w-[22px]'}`}
          />
        </motion.div>
      </div>
      <div className="pt-0.5">
        {hasText(item.line_1) ? (
          <h3 className={`font-body font-black leading-tight text-white ${compact ? 'text-[11px] sm:text-[12px]' : 'text-[12px] sm:text-[13px] lg:text-[14px] lg:max-[1600px]:text-[12px]'}`}>
            {item.line_1}
          </h3>
        ) : null}
        {hasText(item.line_2) ? (
          <p className={`font-body font-semibold leading-[1.42] text-[#9f9f9f] ${compact ? 'mt-1 text-[10px] sm:mt-1.5 sm:text-[11px]' : 'mt-1.5 text-[11px] sm:text-[12px] lg:max-[1600px]:mt-1 lg:max-[1600px]:text-[11px]'}`}>
            {item.line_2}
          </p>
        ) : null}
      </div>
