...
export function HomeWhyChooseUsSection({ section }: HomeWhyChooseUsSectionProps) {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden bg-[#050505] px-5 pb-[92px] pt-[96px] text-white sm:px-8 sm:pb-[108px] sm:pt-[112px] lg:px-12 lg:pb-[126px] lg:pt-[136px] xl:pb-[138px] xl:pt-[150px]">
      ...
      <motion.div
        className="pointer-events-none absolute left-[-82px] top-[26%] z-[3] hidden w-[205px] sm:block lg:left-[-86px] lg:top-[28%] lg:w-[248px] xl:left-[-76px] xl:w-[286px]"
      ...
      <div className="relative z-[2] mx-auto max-w-[1560px]">
        ...
        <div className="mt-[46px] grid items-center gap-7 lg:mt-[62px] lg:grid-cols-[minmax(340px,1fr)_minmax(230px,360px)_minmax(340px,1fr)] lg:gap-8 xl:gap-12">
          <div className="mx-auto w-full max-w-[512px] lg:ml-auto lg:mr-0">
          ...
          <motion.div
            className="mx-auto flex w-[220px] max-w-full items-center justify-center sm:w-[270px] lg:w-[340px]"
          ...
          <div className="mx-auto w-full max-w-[512px] lg:ml-0 lg:mr-auto">
...
function HomeWhyChooseItem({ item, iconFallback, index, align }: { item: AboutUsItem; iconFallback: string; index: number; align: 'left' | 'right' }) {
  ...
  return (
    ...
    >
      <div className="flex h-full min-h-[64px] items-center justify-center border-r border-white/[0.14]">
        <motion.div
        ...
        >
          <Image src={icon} alt={item.icon_alt ?? item.line_1 ?? ''} width={32} height={32} className="h-[23px] w-[23px] object-contain brightness-0 invert sm:h-[26px] sm:w-[26px]" />
        </motion.div>
      </div>
      <div className="pt-0.5">
        {hasText(item.line_1) ? <h3 className="font-body text-[12px] font-black leading-tight text-white sm:text-[13px] lg:text-[14px]">{item.line_1}</h3> : null}
        {hasText(item.line_2) ? <p className="mt-1.5 font-body text-[11px] font-semibold leading-[1.42] text-[#9f9f9f] sm:text-[12px]">{item.line_2}</p> : null}
      </div>
    </motion.article>
  );
}
