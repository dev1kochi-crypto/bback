...
@php
    ...
    $mainImageAccept = $mainImageConfig['accept'] ?? '.png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp';
    $mainImageMimes = implode(', ', $mainImageConfig['mimes'] ?? ['png', 'jpg', 'jpeg', 'webp']);
    $mainImageHelpText = $mainImageConfig['help_text'] ?? 'Upload only the food cutout. Static hero background and decorative assets are handled in the frontend.';
@endphp
<div class="card">
            ...
            <div class="tab-content mb-4 language-switcher-content">
                ...
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="panel-{{ $lang->code }}" role="tabpanel">
                    <div class="row g-4">
                        ...
                        <div class="col-12">
                            <label class="form-label fw-bold">Top Large Word <span class="text-danger">*</span></label>
                            <input type="text" name="translations[{{ $lang->code }}][line_1]" class="form-control @error("translations.{$lang->code}.line_1") is-invalid @enderror" value="{{ old("translations.{$lang->code}.line_1") }}" placeholder="CHICKEN" required>
                            @error("translations.{$lang->code}.line_1")
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text mt-1 text-muted">Large front word in the hero, for example CHICKEN or FAJITA.</div>
                        </div>
                        @endif

                        @if(config('cms-kit.database.banners.items.line_2', true))
                        <div class="col-12">
                            <label class="form-label fw-bold">Bottom Large Word</label>
                            <input type="text" name="translations[{{ $lang->code }}][line_2]" class="form-control @error("translations.{$lang->code}.line_2") is-invalid @enderror" value="{{ old("translations.{$lang->code}.line_2") }}" placeholder="BURGER">
                            @error("translations.{$lang->code}.line_2")
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text mt-1 text-muted">Large background word in the hero, for example BURGER, SANDWICH, or SHAWARMA.</div>
                        </div>
                        @endif

                        @if(config('cms-kit.database.banners.items.content', true))
                        <div class="col-12">
                            <label class="form-label fw-bold">Circle Callout Text</label>
                            <textarea name="translations[{{ $lang->code }}][content]" class="form-control @error("translations.{$lang->code}.content") is-invalid @enderror" rows="3" placeholder="Enjoy great food & special moments with friends & family at B Back Restaurant.">{{ old("translations.{$lang->code}.content") }}</textarea>
                            @error("translations.{$lang->code}.content")
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text mt-1 text-muted">Short text shown inside the round brush callout.</div>
                        </div>
                        @endif
                            ...
                            <div class="buttons-container-{{ $lang->code }} bg-light p-3 rounded-3 mb-2">
                                <div class="button-row row g-2 mb-2">
                                    ...
                                    <div class="col-md-6">
                                     ...
                                     <div class="col-md-5">
                                        <input type="text" name="translations[{{ $lang->code }}][buttons][1][label]" class="form-control" placeholder="Button Label" value="{{ old("translations.{$lang->code}.buttons.1.label") }}">
                                    </div>
                                    <div class="col-md-6">
                                        <input type="text" name="translations[{{ $lang->code }}][buttons][1][url]" class="form-control" placeholder="Button URL" value="{{ old("translations.{$lang->code}.buttons.1.url") }}">
                                    </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6 {{ !in_array('image', $allowedTypes) || (count($allowedTypes) > 1 && old('banner_type', $allowedTypes[0]) !== 'image') ? 'd-none' : '' }}" id="image-section">
                            <label class="form-label">Food Cutout Image <span class="text-danger">*</span></label>
                            <input type="file" name="image" class="form-control @error('image') is-invalid @enderror" accept="{{ $mainImageAccept }}">
                            @error('image')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                            <small class="text-muted d-block mt-1">Recommended max: {{ $mainImageConfig['width'] }}x{{ $mainImageConfig['height'] }}px, {{ $mainImageMimes }} (Max: {{ $mainImageConfig['max_size'] / 1024 }}MB)</small>
                            <small class="text-muted d-block mt-1">{{ $mainImageHelpText }}</small>
                        </div>
                        @endif
