...
class TestimonialController extends Controller
{
    ...
    public function updateSection(Request $request)
    {
        ...
        $rules['section_image'] = array_merge((array) ($rules['section_image'] ?? ['nullable']), $this->imageRules('testimonials.section_image', ['image']));
        $rules['banner'] = array_merge((array) ($rules['banner'] ?? ['nullable']), $this->imageRules('testimonials.banner', ['image']));
        $rules['remove_section_image'] = 'nullable|boolean';
        $rules['remove_banner'] = 'nullable|boolean';

        $request->validate($rules);
        $this->validateImageWithinLimits($request, 'section_image', config('cms-kit.images.testimonials.section_image', []), 'Testimonial section image');
        $this->validateImageWithinLimits($request, 'banner', config('cms-kit.images.testimonials.banner', []), 'Testimonial banner');
    ...
    protected function validateItem(Request $request, array $itemConfig, $languages, ?Testimonial $testimonial = null)
    {
        ...
        $rules['image'] = array_merge((array) ($rules['image'] ?? ['nullable']), $this->imageRules('testimonials.item_image', ['image']));
