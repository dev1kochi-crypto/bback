...
class BannerController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($currentCount >= $maxBanners) {
            return redirect()->route('cms.banners.index')->with('error', 'Maximum 4 banner items allowed.');
        }
    ...
    public function store(Request $request)
    {
        ...
        $mainImageConfig = config('cms-kit.images.banners.main_image', ['max_size' => 2048, 'width' => 1920, 'height' => 800]);
        $mainImageMimes = $mainImageConfig['mimes'] ?? ['png', 'jpg', 'jpeg', 'webp'];
        $avatarConfig = config('cms-kit.images.banners.client_avatar', ['max_size' => 512, 'width' => 100, 'height' => 100]);
        $request->validate([
            'banner_type' => $typeRule,
            'image' => 'nullable|required_if:banner_type,image|image|mimes:' . implode(',', $mainImageMimes) . '|max:' . $mainImageConfig['max_size'],
            ...
            'image.mimes' => 'Banner image must be one of these file types: ' . implode(', ', $mainImageMimes) . '.',
